...
--
-- [MODIFICAR Y MOSTRAR RESULTADO — admin y rrhh]
--   sp_modificar_y_consultar        — Modifica persona y/o legajo usando COALESCE
--                                     (NULL = no modificar ese campo) y devuelve
--                                     toda la info actualizada en result sets
--
-- [MODIFICAR DATOS SECUNDARIOS — admin y rrhh]
--   sp_modificar_titulo             — Modifica titulo y devuelve el registro actualizado
--   sp_modificar_curso              — Modifica curso y devuelve el registro actualizado
--   sp_modificar_idioma             — Modifica nivel/activo de idioma
--   sp_modificar_familiar           — Modifica datos de un familiar
--   sp_modificar_antecedente        — Modifica antecedente laboral
--
-- [BACKUP]
...
DROP PROCEDURE IF EXISTS sp_modificar_y_consultar;
...
-- ============================================================
-- SP: MODIFICAR PERSONA O LEGAJO Y MOSTRAR RESULTADO
-- (admin y rrhh) — modifica y devuelve toda la info actualizada
-- ============================================================
CREATE PROCEDURE sp_modificar_y_consultar(
    IN  p_id_usuario_op  INT,
    IN  p_id_legajo      INT,
    -- Datos persona (NULL = no modificar)
    IN  p_apellido       VARCHAR(50),
    IN  p_nombre         VARCHAR(50),
    IN  p_genero         ENUM('masculino','femenino','sin_determinar'),
    IN  p_fecha_nac      DATE,
    ...
    IN  p_tel_emergencia VARCHAR(20),
    IN  p_email          VARCHAR(50),
    -- Datos legajo (NULL = no modificar)
    IN  p_fecha_ingreso  DATE,
    IN  p_fecha_adm      DATE,
    IN  p_id_cargo       INT,
    IN  p_id_categoria   INT,
...
sp_mod_cons: BEGIN
    ...
    IF v_tipo NOT IN ('administrador', 'rrhh') THEN
        SET p_resultado = 'ERROR: Solo administrador o RRHH pueden modificar datos.';
        LEAVE sp_mod_cons;
    END IF;
    ...
    -- Actualizar solo los campos que vienen con valor (no NULL)
    UPDATE personas SET
        apellido            = COALESCE(p_apellido,       apellido),
        nombre              = COALESCE(p_nombre,         nombre),
        genero              = COALESCE(p_genero,         genero),
        fecha_nacimiento    = COALESCE(p_fecha_nac,      fecha_nacimiento),
        estado_civil        = COALESCE(p_estado_civil,   estado_civil),
        cantidad_hijos      = COALESCE(p_cantidad_hijos, cantidad_hijos),
        provincia_residencia = COALESCE(p_provincia,     provincia_residencia),
        ciudad_residencia   = COALESCE(p_ciudad,         ciudad_residencia),
        domicilio_datos     = COALESCE(p_domicilio,      domicilio_datos),
        cuil                = COALESCE(p_cuil,           cuil),
        telefono            = COALESCE(p_telefono,       telefono),
        telefono_emergencia = COALESCE(p_tel_emergencia, telefono_emergencia),
        email               = COALESCE(p_email,          email)
    WHERE id_persona = v_id_persona;

    UPDATE legajos SET
        fecha_ingreso               = COALESCE(p_fecha_ingreso, fecha_ingreso),
        fecha_ingreso_administracion = COALESCE(p_fecha_adm,   fecha_ingreso_administracion),
        id_cargo                    = COALESCE(p_id_cargo,     id_cargo),
        id_categoria                = COALESCE(p_id_categoria, id_categoria),
        id_oficina                  = COALESCE(p_id_oficina,   id_oficina),
    ...
    INSERT INTO log_sistema (id_usuario, usuario_nombre, tipo_usuario,
        accion, tabla_afectada, id_registro, resultado)
    SELECT p_id_usuario_op, u.usuario, v_tipo,
        'MODIFICAR_Y_CONSULTAR', 'legajos', p_id_legajo, 'exitoso'
    FROM usuario u WHERE u.id_usuario = p_id_usuario_op;
    ...
    -- Devolver toda la info actualizada
    SELECT p.*, l.id_legajo, l.fecha_ingreso, l.fecha_ingreso_administracion,
        l.tipo_contrato, l.estado,
        cat.nombre_categoria, c.nombre_cargo, o.nombre_oficina
    FROM personas p
...
-- ============================================================
-- SPs: MODIFICAR DATOS SECUNDARIOS (admin y rrhh)
-- ============================================================
...
sp_mod_tit: BEGIN
    DECLARE v_tipo VARCHAR(20);
    SELECT tipo INTO v_tipo FROM usuario WHERE id_usuario = p_id_usuario_op AND activo = 1;
    IF v_tipo NOT IN ('administrador','rrhh') THEN
        SET p_resultado = 'ERROR: Solo administrador o RRHH pueden modificar titulos.';
        LEAVE sp_mod_tit;
    END IF;
    UPDATE titulos SET
        nombre_titulo = COALESCE(p_nombre_titulo, nombre_titulo),
        institucion   = COALESCE(p_institucion,   institucion),
...
sp_mod_cur: BEGIN
    ...
    SELECT tipo INTO v_tipo FROM usuario WHERE id_usuario = p_id_usuario_op AND activo = 1;
    IF v_tipo NOT IN ('administrador','rrhh') THEN
        SET p_resultado = 'ERROR: Solo administrador o RRHH pueden modificar cursos.';
        LEAVE sp_mod_cur;
    END IF;
    UPDATE cursos SET
        nombre_curso = COALESCE(p_nombre_curso, nombre_curso),
        institucion  = COALESCE(p_institucion,  institucion),
        fecha_inicio = COALESCE(p_fecha_inicio, fecha_inicio),
...
sp_mod_id: BEGIN
    ...
    SELECT tipo INTO v_tipo FROM usuario WHERE id_usuario = p_id_usuario_op AND activo = 1;
    IF v_tipo NOT IN ('administrador','rrhh') THEN
        SET p_resultado = 'ERROR: Solo administrador o RRHH pueden modificar idiomas.';
        LEAVE sp_mod_id;
    END IF;
    UPDATE idiomas SET
...
sp_mod_fam: BEGIN
    ...
    SELECT tipo INTO v_tipo FROM usuario WHERE id_usuario = p_id_usuario_op AND activo = 1;
    IF v_tipo NOT IN ('administrador','rrhh') THEN
        SET p_resultado = 'ERROR: Solo administrador o RRHH pueden modificar datos familiares.';
        LEAVE sp_mod_fam;
    END IF;
    UPDATE familiar SET
        relacion_empleado         = COALESCE(p_relacion,         relacion_empleado),
        apellido_familiar         = COALESCE(p_apellido_familiar, apellido_familiar),
        nombre_familiar           = COALESCE(p_nombre_familiar,  nombre_familiar),
        dni_familiar              = COALESCE(p_dni_familiar,     dni_familiar),
        fecha_nacimiento_familiar = COALESCE(p_fecha_nac,        fecha_nacimiento_familiar),
        activo                    = COALESCE(p_activo,           activo)
...
sp_mod_ant: BEGIN
    ...
    SELECT tipo INTO v_tipo FROM usuario WHERE id_usuario = p_id_usuario_op AND activo = 1;
    IF v_tipo NOT IN ('administrador','rrhh') THEN
        SET p_resultado = 'ERROR: Solo administrador o RRHH pueden modificar antecedentes laborales.';
        LEAVE sp_mod_ant;
    END IF;
    UPDATE antecedente_laboral SET
        empresa       = COALESCE(p_empresa,      empresa),
        cargo_ocupado = COALESCE(p_cargo,        cargo_ocupado),
        fecha_inicio  = COALESCE(p_fecha_inicio, fecha_inicio),
        fecha_fin     = COALESCE(p_fecha_fin,    fecha_fin),
        activo        = COALESCE(p_activo,       activo)

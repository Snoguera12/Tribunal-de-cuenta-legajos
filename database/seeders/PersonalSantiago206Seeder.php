...
/**
 * Carga masiva de personal "casi real" para probar el sistema completo:
 *  - 10 Funcionarios
 *  -  5 RRHH
 *  -  1 Administrador
 *  - 190 Empleados
 *  Total: 206 personas / usuarios / legajos.
 *
 * TODAS las 206 personas (sin excepción) quedan con: domicilio y teléfono
 * realistas de Santiago del Estero, al menos un Estudio, familiares,
 * cursos, idioma y antecedente laboral. Los nombres, apellidos,
 * localidades, calles, barrios, instituciones y empresas son
 * representativos de la provincia, pero las combinaciones persona-DNI-CUIL
 * son SINTÉTICAS (no corresponden a personas reales existentes). El DNI y
 * el CUIL respetan el formato y el dígito verificador real de AFIP.
 *
 * IMPORTANTE: ejecutar sólo sobre una base de datos vacía en tablas de
 * personal (ver guía de "borrado de datos" antes de correr este seeder).
 */
class PersonalSantiago206Seeder extends Seeder
{
    ...
    /** Contador único que crece con cada persona creada (funcionario, rrhh,
     * administrador o empleado). Se usa como "semilla" determinística para
     * domicilio, teléfono y datos relacionados, así nadie se queda sin
     * información y los datos no se repiten en patrones obvios. */
    private int $seed = 0;

    // ---- Pools de datos realistas de Santiago del Estero ----------------
    ...
    /** Código de área telefónico aproximado por localidad (formato real
     * argentino: 0-CÓDIGO 15-NÚMERO para celulares). */
    ...
    public function run(): void
    {
        ...
        // ------------------------------------------------------------------
        // 1) FUNCIONARIOS (10) — cúpula directiva del organismo
        // ------------------------------------------------------------------
        $funcionarios = [
        ...
        foreach ($rrhh as $indexRrhh => $r) {
            ...
            // El primer RRHH (Silvia Paz) queda registrado como responsable
            // de las bajas del resto del personal.
            if ($indexRrhh === 0) {
        ...
        // ------------------------------------------------------------------
        // 3) ADMINISTRADOR (1) — cuenta de sistema, con legajo propio
        // ------------------------------------------------------------------
        $this->crearPersonal(
        ...
        // ------------------------------------------------------------------
        // 4) EMPLEADOS (190) — generados a partir de "pools" de datos reales
        //    de nombres, apellidos, localidades e instituciones santiagueñas.
        // ------------------------------------------------------------------
        $this->generarEmpleados(190, $numLegajo, $areaIds, $cargoIds, $categoriaIds, $rrhhResponsableEmail);
    }

    // ======================================================================
    // GENERADOR MASIVO DE EMPLEADOS
    // ======================================================================
    private function generarEmpleados(
        ...
        // Cargos y categorías propios de un puesto de "empleado" (no directivos).
        $cargosEmpleado = [
    ...
    // ======================================================================
    // HELPER: crea Persona + User + Legajo (+ Historialbaja si corresponde)
    //         + TODOS los datos relacionados (estudio, familiares, cursos,
    //         idiomas, antecedentes laborales, documentos) para el 100%.
    // ======================================================================
    ...
    /**
     * Genera domicilio y teléfonos realistas según la localidad:
     * calle real + altura + (barrio, si es Capital/La Banda) y un número
     * de celular con el código de área correcto de la zona.
     *
     * @return array{0:string,1:string,2:string} [domicilio, telefono, telefono_emergencia]
     */
    ...
    /**
     * Agrega SIEMPRE (100% de los casos, sin excepción) los datos
     * relacionados de una persona: estudio (+ título si corresponde),
     * familiares, cursos, idioma, antecedente laboral y documentos.
     */
    private function agregarDatosRelacionados(
        ...
        // ------------------------------------------------------------
        // 1) ESTUDIO + TÍTULO — 100% de los casos.
        // ------------------------------------------------------------
        if ($esCupula) {
        ...
        // ------------------------------------------------------------
        // 2) FAMILIARES — 100% de los casos (1 a 3 familiares).
        // ------------------------------------------------------------
        $this->crearFamiliaresDemo($persona->id, $seed);

        // ------------------------------------------------------------
        // 3) CURSOS DE CAPACITACIÓN — 100% (1 o 2 cursos).
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // 4) IDIOMA — 100% (1 idioma, y una parte tiene un segundo).
        // ------------------------------------------------------------
        Idioma::create([
            'persona_id' => $persona->id,
        ...
        // ------------------------------------------------------------
        // 5) ANTECEDENTE LABORAL — 100% (todos tuvieron un trabajo previo).
        // ------------------------------------------------------------
        ...
        // ------------------------------------------------------------
        // 6) DOCUMENTOS — Curriculum para el 100%, Licencia para ~1 de cada 5.
        // ------------------------------------------------------------
        Documento::create([
    ...
    /**
     * Calcula el CUIL con el dígito verificador real de AFIP.
     * Prefijo: 20 (varón), 27 (mujer). Devuelve [cuilCompleto, prefijoFinal].
     */

...
-- TRIGGER #3:  trg_historico_personas_delete     - Guarda antes de un DELETE

-- 2. TABLA: legajos
-- TRIGGER #4:  trg_historico_legajos_insert      - Guarda luego de un INSERT
-- TRIGGER #5:  trg_historico_legajos_update      - Guarda antes de un UPDATE
-- TRIGGER #6:  trg_historico_legajos_delete      - Guarda antes de un DELETE

-- 3. TABLA: estudio
-- TRIGGER #7:  trg_historico_estudio_insert      - Guarda luego de un INSERT
-- TRIGGER #8:  trg_historico_estudio_update      - Guarda antes de un UPDATE
-- TRIGGER #9:  trg_historico_estudio_delete      - Guarda antes de un DELETE

-- 4. TABLA: curso
-- TRIGGER #10: trg_historico_curso_insert        - Guarda luego de un INSERT
-- TRIGGER #11: trg_historico_curso_update        - Guarda antes de un UPDATE
-- TRIGGER #12: trg_historico_curso_delete        - Guarda antes de un DELETE

-- 5. TABLA: idioma
-- TRIGGER #13: trg_historico_idioma_insert       - Guarda luego de un INSERT
-- TRIGGER #14: trg_historico_idioma_update       - Guarda antes de un UPDATE
-- TRIGGER #15: trg_historico_idioma_delete       - Guarda antes de un DELETE

-- 6. TABLA: familiar
-- TRIGGER #16: trg_historico_familiar_insert     - Guarda luego de un INSERT
-- TRIGGER #17: trg_historico_familiar_update     - Guarda antes de un UPDATE
-- TRIGGER #18: trg_historico_familiar_delete     - Guarda antes de un DELETE

-- 7. TABLA: antecedente_laboral
-- TRIGGER #19: trg_historico_antecedente_laboral_insert   - Guarda luego de un INSERT
-- TRIGGER #20: trg_historico_antecedente_laboral_update   - Guarda antes de un UPDATE
-- TRIGGER #21: trg_historico_antecedente_laboral_delete   - Guarda antes de un DELETE

-- 8. TABLA: historial_legajo
-- TRIGGER #22: trg_historico_historial_legajo_insert      - Guarda luego de un INSERT
-- TRIGGER #23: trg_historico_historial_legajo_update      - Guarda antes de un UPDATE
-- TRIGGER #24: trg_historico_historial_legajo_delete      - Guarda antes de un DELETE

-- 9. TABLA: sumario
-- TRIGGER #25: trg_historico_sumario_insert       - Guarda luego de un INSERT
-- TRIGGER #26: trg_historico_sumario_update       - Guarda antes de un UPDATE
-- TRIGGER #27: trg_historico_sumario_delete       - Guarda antes de un DELETE

-- 10. TABLA: documentos
-- TRIGGER #28: trg_historico_documento_insert     - Guarda luego de un INSERT
-- TRIGGER #29: trg_historico_documento_update     - Guarda antes de un UPDATE
-- TRIGGER #30: trg_historico_documento_delete     - Guarda antes de un DELETE

-- TRIGGER #31: tgr_borrado_logico_total           - Cuando un legajo se da de baja, desactiva TODOS los registros relacionados en todas las tablas (activo = 0)
...
DROP TRIGGER IF EXISTS trg_historico_antecedente_laboral_delete;
...
CREATE TRIGGER trg_historico_personas_delete
...
CREATE TRIGGER trg_historico_legajos_delete
...
CREATE TRIGGER trg_historico_titulos_delete
BEFORE DELETE ON titulos
FOR EACH ROW
BEGIN
    INSERT INTO historico_titulos (
        id_titulo, titulo, nivel_estudio, institucion, fecha_inicio, fecha_fin, id_legajo,
...
CREATE TRIGGER trg_historico_cursos_delete
...
CREATE TRIGGER trg_historico_idiomas_delete
BEFORE DELETE ON idiomas
FOR EACH ROW
BEGIN
    INSERT INTO historico_idiomas (
        id_idioma, nombre, nivel, id_legajo,
...
CREATE TRIGGER trg_historico_familiar_delete
...
CREATE TRIGGER trg_historico_antecedente_laboral_delete
BEFORE DELETE ON antecedente_laboral
FOR EACH ROW
BEGIN
    INSERT INTO historico_antecedente_laboral (
        id_antecedente, empresa, cargo, fecha_inicio, fecha_fin, id_legajo,
...
CREATE TRIGGER trg_historico_historial_legajos_delete
BEFORE DELETE ON historial_legajos
FOR EACH ROW
BEGIN
    INSERT INTO historico_historial_legajos (
        id_historial, fecha_registro, accion, detalle, id_legajo,
...
BEGIN
    INSERT INTO historico_sumarios (
        id_sumario, fecha_registro, id_legajo,
        usuario_accion, tipo_cambio
    ) VALUES (
        NEW.id_sumario, DATE(NEW.fecha_registro), NEW.id_legajo,
        USER(), 'INSERT'
    );
END//
...
BEGIN
    INSERT INTO historico_sumarios (
        ...
        OLD.id_sumario, DATE(OLD.fecha_registro), OLD.id_legajo,
        USER(), 'UPDATE'
    );
END//

CREATE TRIGGER trg_historico_sumarios_delete
BEFORE DELETE ON sumarios
FOR EACH ROW
BEGIN
    INSERT INTO historico_sumarios (
        id_sumario, fecha_registro, id_legajo,
...
CREATE TRIGGER trg_historico_documentos_delete
...
-- ======================================================================
-- 11. TRIGGER #31: BORRADO Y REACTIVACIÓN LÓGICA GLOBAL
-- ======================================================================
...
FOR EACH ROW
BEGIN
    -- Caso A: El legajo pasa a estar 'de_baja'
    ...
    -- Caso B: El legajo se reactiva volviendo a estar 'activo'
